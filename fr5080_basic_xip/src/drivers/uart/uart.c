...
__attribute__((section("need_kept"))) int uart_get_baud_divisor(uint8_t baudrate)
{
	int Tmp32;
...
__attribute__((section("need_kept"))) void uart_reset_register(void)
{
...
__attribute__((section("need_kept"))) void uart_init(uint8_t bandrate, void (*callback)(uint8_t))
{
...
__attribute__((section("need_kept"))) void uart_flush_rxfifo_noint(void)
{
...
__attribute__((section("need_kept"))) void uart_finish_transfers(void)
{
    /* wait for tx fifo is empty */
    while(!(uart_reg->lsr & 0x40));
}

__attribute__((section("need_kept"))) void uart_putc_noint(uint8_t c)
{
    while (!(uart_reg->lsr & 0x40));
    uart_reg->u1.data = c;
}

__attribute__((section("need_kept"))) void uart_puts_noint(const char *s)
{
	uint32_t count;
...
__attribute__((section("need_kept"))) void uart_putc_noint_no_wait(uint8_t c)
{
    uart_reg->u1.data = c;
}

__attribute__((section("need_kept"))) void uart_put_data_noint(const uint8_t *d, int size)
{
	uint32_t count;
...
__attribute__((section("need_kept"))) void uart_get_data_noint(uint8_t *buf, int size)
{
	while (1) {
...
__attribute__((section("need_kept"))) int uart_get_data_nodelay_noint(uint8_t *buf, int size)
{
	int count = 0;
...
__attribute__((section("need_kept"))) void uart_write(uint8_t *bufptr, uint32_t size, void (*callback) (uint8_t))
{
    uint32_t count;
