...
const struct qspi_stig_reg_t read_status_3_cmd = {
    .enable_bank = 0,
    .dummy_cycles = 0,
    .write_bytes = 0,
    .enable_write = 0,
...
const struct qspi_stig_reg_t write_status_31_cmd = {
    .enable_bank = 0,
    .dummy_cycles = 0,
    .write_bytes = 1,
    .enable_write = 1,
...
const struct qspi_stig_reg_t write_status_11_cmd = {
    .enable_bank = 0,
    .dummy_cycles = 0,
    .write_bytes = 1,
    .enable_write = 1,
...
__attribute__((section("need_kept"))) void qspi_flash_enable_quad(uint8_t flag)
{
    uint8_t status[2];
    ...
    if(flag == 2) {
        ...
        status[1] |= 0x04;  //enable quad mode
    }
    else {
    ...
    switch(flag) {
        ...
        case 1:
        case 2:
        qspi_stig_cmd(write_status_31_cmd, QSPI_STIG_CMD_WRITE, 1, &status[1]);
            break;
        default:
            return;
    }
...
__attribute__((section("need_kept"))) void qspi_flash_enhance_drive(uint8_t config)
{
    uint8_t status[1];

    qspi_stig_cmd(read_status_3_cmd, QSPI_STIG_CMD_READ, 1, &status[0]);
    if((status[0] & 0x60) == config) {
        return;
    }
    status[0] &= 0x9f;
    status[0] |= config;
    ...
    qspi_stig_cmd(write_enable_cmd, QSPI_STIG_CMD_EXE, 0, 0);
    qspi_stig_cmd(write_status_11_cmd, QSPI_STIG_CMD_WRITE, 1, &status[0]);
