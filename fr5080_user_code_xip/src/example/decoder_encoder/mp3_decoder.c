...
#define MP3_DECODER_BUFFER_SIZE         4096
...
static void (*pcm_data_handler)(uint8_t *data, uint32_t length);
...
static void mp3_decoder_engine_decode(void)
{
    ...
    if(i_out_bytes) {
        ...
        if(resample_used) {
            uart_putc_noint_no_wait('[');
#if USE_DOWNSAMPLE == 1
            downsample_entity(pb_out_buf, i_out_bytes, pcm_data_handler);
#else
            upsample_entity(pb_out_buf, i_out_bytes, pcm_data_handler);
#endif
            uart_putc_noint_no_wait(']');
        }
        else {
            pcm_data_handler(pb_out_buf, i_out_bytes);
        }
...
error_exit:
    ...
    MP3_LOG("mp3_decoder_dynamic_consumed is %d, %d.\r\n", mp3_decoder_dynamic_consumed, mp3_decoder_all_request_data_received);
    ...
    if((mp3_decoder_all_request_data_received == true)
        ...
        if(1) {
            ...
            GLOBAL_INT_RESTORE();
            uint32_t apply_unit = (MP3_DECODER_BUFFER_SIZE-left_space)/256 - 1;
//            if(apply_unit >= 10) {
//                uart_putc_noint('a'+apply_unit-10);
//            }
//            else {
//                uart_putc_noint('0'+apply_unit);
//            }
            if(apply_unit) {
                ...
                //ipc_insert_msg(channel, IPC_MSG_RAW_BUFFER_SPACE, apply_unit, NULL);
                ipc_msg_send(IPC_MSG_RAW_BUFFER_SPACE, apply_unit, NULL);
            }
        }
    }
...
void mp3_decoder_init(void *arg)
{
    uint8_t channel;
    uint8_t *param = arg;
    struct task_msg_t *msg = arg;
    uint8_t *type = (void *)&msg->param[0];
    ...
    if(1) {
        ...
        uint32_t apply_unit = MP3_DECODER_BUFFER_SIZE/256 - 1;
        //ipc_insert_msg(channel, IPC_MSG_RAW_BUFFER_SPACE, apply_unit, NULL);
        ipc_msg_send(IPC_MSG_RAW_BUFFER_SPACE, apply_unit, NULL);
        ...
        if(*type) {
            pcm_data_handler = native_playback_recv_frame;
            native_playback_init();
        }
        else {
            pcm_data_handler = sbc_encoder_recv_frame;
        }
    }
}
