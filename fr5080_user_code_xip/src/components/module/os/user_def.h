...
enum ipc_user_sub_msg_type_t {
    ...
    IPC_SUB_MSG_DECODER_STOP,   // MCU notice DSP to stop decoder
    ...
    IPC_SUB_MSG_DECODER_START_LOCAL,// MCU notice DSP DSP to initiate MP3, AAC, etc. decoder for native playback
};
...
enum ipc_user_msg_type_t {
    ...
    IPC_MSG_RAW_SBC_FRAME = 11,
};

enum ipc_msg_flash_opcode_t {
	IPC_MSG_FLASH_ERASE,
	IPC_MSG_FLASH_ERASE_ACK,
	IPC_MSG_FLASH_READ,
	IPC_MSG_FLASH_READ_ACK,
	IPC_MSG_FLASH_WRITE,
	IPC_MSG_FLASH_WRITE_ACK,
	IPC_MSG_FLASH_COPY,
	IPC_MSG_FLASH_COPY_ACK,			// reply to MCU with this message when an error occurs, otherwise IPC_SUB_MSG_FLASH_COPY_ACK will be used
	IPC_MSG_FLASH_CHECK_CRC,
	IPC_MSG_FLASH_CHECK_CRC_ACK,
};

enum ipc_msg_flash_status_t {
	IPC_MSG_FLASH_OK,
	IPC_MSG_FLASH_ERROR,
};

struct ipc_msg_flash_op_t {
	uint8_t opcode;
	uint8_t status;
	uint16_t reserved;
	uint32_t length;
