...
{
    GLOBAL_INT_DISABLE();
    co_list_push_front(&task_list, (struct co_list_hdr *)task_msg);
    GLOBAL_INT_RESTORE();
}

__attribute__((section("iram_section"))) void *task_msg_get(void)
{
    void *msg;
...
__attribute__((section("iram_section"))) void task_schedule(const struct task_msg_handler_t *func_table, uint32_t table_size)
{
    struct task_msg_t *msg;
    task_func_t func;
