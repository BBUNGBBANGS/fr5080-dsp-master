...
void lv_port_disp_init(void)
{
    ...
    static lv_color_t buf1_1[LV_HOR_RES_MAX * 30];                      /*A buffer for 10 rows*/
    lv_disp_buf_init(&disp_buf_1, buf1_1, NULL, LV_HOR_RES_MAX * 30);   /*Initialize the display buffer*/
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
	...
	hight	= area->y2 - area->y1 + 1;

	len = width*hight*2;

	lcd_gc9c01_set_windows(area->x1,
					area->y1,
					area->x2 - 0,
					area->y2 - 0);

	lcd_gc9c01_display((uint8_t *)color_p, (len));

	//debug
	//printf("area=%d %d %d %d\r\n",area->x1, area->x2, area->y1,area->y2);
	//printf("disp_flush=%d %d %d \r\n",width, hight, len);
