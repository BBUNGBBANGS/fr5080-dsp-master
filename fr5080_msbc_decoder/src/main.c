...
void msbc_decoder_do_decoder_handler(void);
...
static void ipc_rx_user_handler(struct ipc_msg_t *msg, uint8_t chn)
{
    ...
    switch(msg->format) {
        case IPC_MSG_RAW_FRAME:
            msbc_decoder_recv_frame(ipc_get_buffer_offset(IPC_DIR_MCU2DSP, chn), msg->length);
            break;
...
static void user_task_handler(struct task_msg_t *msg)
{
    switch(msg->id) {
        ...
        case MP3_DECODER_DO_DECODE:
            msbc_decoder_do_decoder_handler();
            break;
