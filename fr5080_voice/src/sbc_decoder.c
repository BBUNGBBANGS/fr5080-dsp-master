...
#define sbc_decoder_malloc(size)        pvPortMalloc_user(size)
#define sbc_decoder_free(ptr)           vPortFree_user(ptr)

enum sbc_decoder_state_t {
    SBC_DECODER_IDLE,
    SBC_DECODER_BUFFERING,
    SBC_DECODER_INITIATING,
    SBC_DECODER_DECODING,
};

enum sbc_decoder_task_id_t {
...
int sbc_decoder_buffer_input_index = 0;
int sbc_decoder_buffer_output_index = 0;
int sbc_decoder_total_input = 0;

pVOID g_pv_arr_alloc_memory[MAX_MEM_ALLOCS];
WORD  g_w_malloc_count = 0;

pWORD8 pb_inp_buf = 0;
pWORD8 pb_out_buf = 0;
UWORD32 ui_inp_size = 0;
UWORD32 i_buff_size = 0;
...
void sbc_decoder_do_decoder_handler(void);
...
static void sbc_decoder_task_handler(struct task_msg_t *msg)
{
    switch(msg->id) {
        case SBC_DECODER_DO_DECODE:
            sbc_decoder_do_decoder_handler();
            break;
        default:
            break;
    }
...
void sbc_decoder_init(void)
{
    ...
    app_register_default_task_handler(sbc_decoder_task_handler);
}
...
void sbc_decoder_do_decoder_handler(void)
{
    switch(sbc_decoder_state) {
        case SBC_DECODER_IDLE:
            sbc_decoder_state = SBC_DECODER_BUFFERING;
        case SBC_DECODER_BUFFERING:
            if(SBC_DECODER_BUFFERING_LIMIT <= sbc_decoder_total_input) {
                sbc_decoder_engine_init();
                sbc_decoder_state = SBC_DECODER_INITIATING;
            }
            else {
                break;
            }
        case SBC_DECODER_INITIATING:
