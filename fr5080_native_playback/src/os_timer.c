...
static void os_timer_check_list(void)
{
...
//	begin_count = xthal_get_ccount();
//	do {
//	    trigger_value += OS_TIMER_PRECISION_COUNTER;
//	    uint32_t last_counter = trigger_value - begin_count;
//	    if((last_counter < 2*OS_TIMER_PRECISION_COUNTER)
//	        && (last_counter > (OS_TIMER_PRECISION_COUNTER>>1))) {
//	        break;
//	    }
//	} while(1);
//	xthal_set_ccompare(XCHAL_TIMER1, trigger_value);

	//lv_tick_inc(OS_TIMER_PRECISION);
}

void os_timer_trigger_handler(struct task_msg_t *msg)
{
...
void os_timer_engine_init(void)
{
	...
	begin_count = xthal_get_ccount();
	trigger_value = begin_count + OS_TIMER_PRECISION_COUNTER;
	xthal_set_ccompare(XCHAL_TIMER1, begin_count + OS_TIMER_PRECISION_COUNTER);
	GLOBAL_INT_RESTORE();
